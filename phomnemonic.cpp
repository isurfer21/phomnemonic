...
void main()
{
 ...
 if(!infile){ cout<<"Can't open the file!"; exit(0);}
 while(!infile.eof())
 {
  for(int i=0;i<15;i++)
  {
 ...
 int ktr=0;
 ltn=strlen(tn);
 for(int kctr=0;kctr<ltn;kctr++)
 {
  if((tn[kctr]>='0' && tn[kctr]<='9') || tn[kctr]=='-')
   bos[ktr++]=tn[kctr];
 }
 for(kctr=0;kctr<ktr;kctr++)
 {
  tn[kctr]=bos[kctr];
 }
 ltn=ktr;
 ...
 for(int z=ltn-1;z>=0;--z)
 {
  ...
  switch(tn[z])
  {
   case '2': a[i]=65; y[i]=3; break;
   case '3': a[i]=68; y[i]=3; break;
   case '4': a[i]=71; y[i]=3; break;
   case '5': a[i]=74; y[i]=3; break;
   case '6': a[i]=77; y[i]=3; break;
   case '7': a[i]=80; y[i]=4; break;
   case '8': a[i]=84; y[i]=3; break;
   case '9': a[i]=87; y[i]=4; break;
   case '0':
   case '1': a[i]=tn[z]; y[i]=1; break;
   default : a[i]=tn[z]; y[i]=1;
  }
 }
                 ...
                 for(int o=0; o<y[12]; o++)
                 {
                  ...
                  for(int p=0; p<y[13]; p++)
                  {
                    ...
                    for(int q=0; q<y[14]; q++)
                    {
                     ...
                     {
                      a[15]+=r;
                      for(int i=15-ltn;i<15;i++) out<<a[i];
                      ctr++;
                      if(ctr%6==0) out<<"\n"; else out<<"\t";
                      a[15]-=r;
                     }
